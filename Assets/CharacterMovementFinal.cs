...
public class CharacterMovementFinal : MonoBehaviour {
	...
	void HorizontalCollisions()
	{
		...
		{
			SceneElementFinal sceneElementFinal = objectToOrificate.GetComponent<SceneElementFinal>();
			Transform currentTransform = objectToOrificate.transform;
			while (!sceneElementFinal && currentTransform.parent != null)
			{
				currentTransform = currentTransform.parent;
				sceneElementFinal = currentTransform.gameObject.GetComponent<SceneElementFinal>();
			}
			if (sceneElementFinal)
			{
				sceneElementFinal.TurnIntoGold();
				if (sceneElementFinal.othersToOrificate != null)
					foreach (SceneElementFinal s in sceneElementFinal.othersToOrificate)
						s.TurnIntoGold();
			}   
		}
	}
	...
	void KillPlantEntity()
	{
		if (!isDead)
		{
			...
			FinalLevelManager.levelManager.RespawnPlayer();
